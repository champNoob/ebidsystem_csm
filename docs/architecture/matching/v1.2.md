...
### 撮合事件 `ev` 只用*值类型*，而非引用类型

### 当前版本的架构：Go 并发 + 事件驱动

服务层 submit → engine.orderCh → 撮合 → eventCh → service 监听 → 更新订单

### 不直接关联 user_id / symbol

因为 Order 已经包含（通过 order_id 就能反查所有信息）
...
func (a *App) Start() {
	a.Engine.Start()
	a.OrderService.StartMatchEventListener()
}
```