# 主要版本

## `v0.1` 用户侧+订单侧

### `0.1.1` “控制器-服务层-模型层”（`CSM`）三层架构

### `0.1.2` 用户侧

### `0.1.3` 订单侧

### `0.1.4` 架构升级

由 `CSM` 架构转为“handler-service-repository-model”四层架构

- 添加 `DTO`（`Request`） 

- 初步错误封装：添加服务层的 `errors.go`

## `v0.2` 撮合引擎

### `0.2.1` 撮合引擎初步（v1.1~v1.2）

- 引擎核心功能实现，支持完全成交

- `main.go` 装配引擎组件

- 服务层调用 `matching.Submit` 接口处理订单

不包含部分成交、撤单等

### `0.2.2` 撮合引擎（v1.3~v1.4）

- 支持部分成交

- 支持撤单

- 订单状态机

- 撮合事件 DB 事务化

- 引擎重启恢复机制

### `v0.2.3` 订单状态机

## `v0.3` 前后端联调 + 功能完善 + 撮合引擎升级

### `v0.3.1` 前后端联调

- CORS

- 前后端接口文档

- 订单查询：历史订单/当前订单

### `v0.3.2` 统一错误封装

- 服务层自定义错误结构体（BusinessError）

- 分层封装错误提示信息（内部错误不展示、服务层管理层分离）

- 自定义错误码（DTO Response + 状态码映射表）

详见 `docs\architecture\errorsPackaging.md`

### `v0.3.3` 多协程撮合引擎

- 重构撮合引擎结构：引擎（Engine）→ 股票代码级撮合器（symbol_matcher）→ 订单簿（OrderBook）

### `v0.3.4` 撮合事件事务化 + 幂等落库 + 引擎优化

- 在服务层、仓储层多方法确保事务的隔离性、原子性和一致性（未作持久化保障）

- 确保 `EventID` 作为唯一索引（`SymbolMatcher`唯一生成）

- 实现价格优先策略

- 引擎优化（上下文+`Stop()`）

### `v0.3.5` 并发压力测试

- 卖单并发提交压力测试

- 纯撮合并发压力测试（单股票代码）

- 多代码撮合并发压力测试

### `v0.3.6` 统一日志系统

- 在 `pkg/logger` 包中封装日志系统，对外暴露 `Log` 和 `Close` 方法

- 将撮合引擎、服务层重要信息等接入日志系统

## `v0.4` 功能完善 + 系统测试

### `v0.4.1` 管理员接入

- admin 路由组（控制层-服务层-仓储层-DTO）、鉴权

- 查看指定用户、创建用户、【查看全部订单】

- Dashboard 总览、Symbol 统计、订单状态分布、用户角色分布、用户交易排行、最近成交记录、成交时间趋势

### `v0.4.2` 重启订单恢复 + 错误封装优化

#### 重启订单恢复机制

- 仓储层`FindActiveOrdersForRecovery`方法
- 服务层`RecoverActiveOrders`方法并在main.go中调用

### 统一错误封装2.0

- 创建`internal/apperror`目录
- 将服务层错误定义移到 `catalog.go`
- 将控制层的 HTTP 状态码映射移到 `http_status.go`
- 更改相关文件的依赖包从`service`到`apperror`

### `v0.4.3` 订单状态机

### `v0.4.4` 市价单
